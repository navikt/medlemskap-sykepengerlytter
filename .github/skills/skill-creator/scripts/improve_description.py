...
by calling the configured LLM backend.
"""
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for an AI coding assistant skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that the AI sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = call_llm(prompt, model=model, timeout=300)
    ...
    # Safety net: if the description is too long, ask for a shorter rewrite
    if len(description) > 1024:
        shorten_prompt = (
            f"The following skill description is {len(description)} characters, "
            f"which exceeds the 1024-character hard limit:\n\n"
            f'"{description}"\n\n'
            f"Rewrite it to be under 1024 characters while keeping the most "
            f"important trigger words and intent coverage. Respond with only "
            f"the new description in <new_description> tags."
        )
        shorten_text = call_llm(shorten_prompt, model=model, timeout=120)
