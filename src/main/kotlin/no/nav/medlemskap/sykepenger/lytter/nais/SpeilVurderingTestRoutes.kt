...
fun Routing.speilVurderingTestRoutes(
        ...
        route("test") {
            authenticate("azureAuth") {
                post("speilvurdering") {
                    ...
                    logger.info(teamLogs, "SpeilVurderingTestRoutes: azp-claim i principal-token: {} ", azp)
                    val callId = call.callId ?: UUID.randomUUID().toString()
                    logger.info(
                        "kall autentisert, url : /test/speilvurdering",
                        kv("callId", callId),
                        kv("endpoint", "test/speilvurdering")
                    )
